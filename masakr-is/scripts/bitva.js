function Timer() {
    ...
    this.interval = window.setInterval(this.regularRefresh, 10 * 1000)
}
...
function Connection() {
    ...
    this.makeRequest = (data) => {
        ...
        this.httpRequest.open('POST', 'https://masakr.instruktori.cz/bitva/refresh', true)
...
function writeShape(actives) {
    let count = 5
    let size = 30
    document.write(`<svg width="${5 * size}" height="${5 * size}">`)
    for (let i = 0; i < count; i++) {
        for (let j = 0; j < count; j++) {
            if (actives.includes(j * count + i)) {
                document.write(`<rect x="${i * size}" y="${j * size}" width="30" height="30" fill="blue" stroke="white" stroke-width="1" />`)
            }
        }
    }
    document.write('</svg>')
